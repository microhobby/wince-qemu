...
void process_event(struct virtio_input_event *ev)
{
	...
	// vga
	DWORD dx = (long long)input_buf.x * 640 / (32768 / 4);
	DWORD dy = (long long)input_buf.y * 480 / (32768 / 4);
	...
	// } else if (ev->type == EV_KEY) {
	// 	if (ev->code == BTN_LEFT) {
	// 		if (ev->value == 1 && last_button == 0) {
	// 			// Handle touch down event
	// 			mouse_event(MOUSEEVENTF_TOUCH | MOUSEEVENTF_LEFTDOWN, dx, dy, 0, 0);
	// 		} else if (ev->value == 0 && last_button == 1) {
	// 			// Handle touch up event
	// 			mouse_event(MOUSEEVENTF_TOUCH | MOUSEEVENTF_LEFTUP, dx, dy, 0, 0);
	// 		}
	// 		last_button = ev->value;
	// 	}
	// }
#endif
}
